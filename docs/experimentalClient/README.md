...
We are actively working on a client librarty that takes care of managing a wallet contract (or the relay hub) on your behalf. It works with [ethersjs v4](https://github.com/ethers-io/ethers.js/) to add the ethers Signer or Contract objects.
...
```ts
const userWalet = new Wallet("<private key>");
